...
class PatientCaseActiveActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_patient_case_active)
        ...
        navCases.setOnClickListener {
            // Already here (in a sub-section of Cases)
        }
    ...
    private fun fetchCases() {
        ...
        com.simats.smileai.network.RetrofitClient.instance.getPatientActiveCases(patientId).enqueue(object : retrofit2.Callback<List<com.simats.smileai.network.Case>> {
            override fun onResponse(call: retrofit2.Call<List<com.simats.smileai.network.Case>>, response: retrofit2.Response<List<com.simats.smileai.network.Case>>) {
                if (response.isSuccessful) {
                    ...
                    findViewById<TextView>(R.id.tabActive)?.text = "Active (${cases.size})"
                    updateCasesUi(cases)
                } else {
                    val error = response.errorBody()?.string() ?: "Unknown error"
                    android.widget.Toast.makeText(this@PatientCaseActiveActivity, "Failed to load: $error", android.widget.Toast.LENGTH_LONG).show()
                }
            }

            override fun onFailure(call: retrofit2.Call<List<com.simats.smileai.network.Case>>, t: Throwable) {
                android.widget.Toast.makeText(this@PatientCaseActiveActivity, "Network Error: ${t.message}", android.widget.Toast.LENGTH_LONG).show()
            }
        })
    }
